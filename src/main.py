...
class Trainer():
    ...
    def train_array(self, model, tr_loader, va_loader, device, adv_train=False):
    ...
    def test_array(self, model, loader, device, adv_test=False,valid=False, criterion=None):
        model.eval()
        logger = self.logger
        if valid==False:
            logger.info("Test Set: %d" % len(loader.dataset))
